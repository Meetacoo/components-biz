...
const BaseExample = createWithRemoteLoader({
  ...
  return <PureGlobal preset={{
    ...
    <div style={{ width: '769px', margin: '0 auto' }}>
      <Fetch {...Object.assign({}, mockApis.contract.getSubjectList)} render={({ data }) => {
        return <Flex vertical gap={10}>
          {data.filter((item) => item.citable).map((item, index) => {
            return <div key={index} dangerouslySetInnerHTML={{ __html: templateRenders.renderCompany(item) }} />;
          })}
        </Flex>;
      }} />
      <Divider />
      <Fetch {...Object.assign({}, mockApis.contract.getBankData)} render={({ data }) => {
        return <Flex vertical gap={10}>
          {data.map((item, index) => {
      ...
      <Divider />
      <Fetch {...Object.assign({}, mockApis.payment.getPaymentData)} render={({ data }) => {
      ...
      <Fetch {...Object.assign({}, mockApis.contract.getBankData)} render={({ data }) => {
        ...
        return <div dangerouslySetInnerHTML={{
          __html: templateRenders.renderTable({
            bankInfo, bankInfoOperation: 'xxxx', totalAmount: '1000', projects: [{
              'id': 223,
              'noticeId': 122,
              'projectTypeId': 7,
              'projectType': '入职到岗',
      ...
      <Fetch {...Object.assign({}, mockApis.contract.getSubjectList)} render={({ data }) => {
        ...
        return <Fetch {...Object.assign({}, mockApis.contract.getBankData)} render={({ data }) => {
          const bankInfo = data[0];
          return <div dangerouslySetInnerHTML={{
